...
async function getSettings() {
  ...
  const managedActive = Array.isArray(managed.sites) && managed.sites.length > 0;
  const sites = managedActive ? managed.sites : stored.sites;
  return { sites: normalizeSites(sites, sites.length > 0 ? [] : stored.patterns), managed: managedActive };
}

function normalizeSites(sites, legacyPatterns = []) {
  const source = Array.isArray(sites) && sites.length > 0 ? sites : normalizePatterns(legacyPatterns).map((pattern) => ({ pattern }));
  return source.map(normalizeSite).filter((site) => site.pattern);
}

function normalizeSite(site) {
  const timeoutSeconds = normalizeTimeout(site.timeoutSeconds);
  return { pattern: String(site.pattern || "").trim(), timeoutSeconds, warningSeconds: normalizeWarning(site.warningSeconds, timeoutSeconds), fromTime: normalizeTime(site.fromTime), toTime: normalizeTime(site.toTime), soundEnabled: site.soundEnabled !== false };
}

function normalizeTime(value) { return /^([01]\d|2[0-3]):[0-5]\d$/.test(String(value || "")) ? value : ""; }
...
function rulePriority(pattern) {
  ...
  if (/^[a-z0-9.-]+$/i.test(value)) score += 5000;
...
async function getSiteForTab(tab) {
  ...
  if (sites.length === 0 || await isProtectionPaused()) return null;
  const matchingSites = sites.map((site, index) => ({ site, index, score: rulePriority(site.pattern) }))
    .filter(({ site }) => patternMatchesUrl(site.pattern, tab.url) && isWithinSchedule(site))
    .sort((left, right) => right.score - left.score || left.index - right.index);
  if (matchingSites.length > 0) return matchingSites[0].site;
...
async function getPauseUntil() {
  ...
  const value = Number(stored[PAUSE_KEY]);
  if (!Number.isFinite(value) || value <= Date.now()) {
    if (value) await chrome.storage.local.remove(PAUSE_KEY);
    return 0;
  }
  return value;
}

async function isProtectionPaused() { return (await getPauseUntil()) > Date.now(); }
...
async function appendAudit(event, metadata = {}) {
  ...
  Object.entries(metadata).forEach(([key, value]) => {
    if (["password", "passwordHash", "salt", "token", "secret", "url", "pattern", "title"].includes(key)) return;
  ...
  const next = [{ timestamp: Date.now(), event, metadata: safeMetadata }, ...current].slice(0, MAX_AUDIT_ENTRIES);
...
async function broadcastStatus() {
  ...
  await Promise.all(tabs.map(async (tab) => {
    ...
    chrome.tabs.sendMessage(tab.id, { type: STATUS_MESSAGE, scheduleActive: true, timeoutSeconds: site.timeoutSeconds, warningSeconds: site.warningSeconds, soundEnabled: site.soundEnabled, idleState: isWarning ? "warning" : "active", remainingSeconds, deadlineAt, isActiveTab: activeTabIds.has(tab.id), hasTabActivity: remainingSeconds > 0, paused: false }).catch(() => {});
  }));
}
...
async function getPopupState() {
  ...
  if (!pauseUntil) {
    for (const tab of tabs) {
      ...
      const lastActivity = await getLastActivity(tab.id);
      const deadlineAt = Math.max(lastActivity, getScheduleStart(site)) + site.timeoutSeconds * 1000;
      monitoredTabs.push({ tabId: tab.id, title: tab.title || "Monitored tab", deadlineAt });
    }
  }
...
async function getAdminState() {
  ...
  return { managed, source: managed ? "Managed policy" : "Local settings", precedence: "Managed policy > local settings", ruleCount: sites.length, monitoredCount: (await getPopupState()).monitoredTabs.length, paused: await isProtectionPaused(), auditCount: (await getAuditLog()).length };
}

async function handleDeadlineSignal(tabId) { await closeTabIfStillInactive(tabId); }
async function configureIdleDetection() { await chrome.alarms.clear(CHECK_ALARM); chrome.alarms.create(CHECK_ALARM, { periodInMinutes: 0.5 }); }
async function clearAllTabAlarmState() {
  const alarms = await chrome.alarms.getAll();
  await Promise.all(alarms.filter((alarm) => alarm.name.startsWith(WARNING_ALARM_PREFIX) || alarm.name.startsWith(CLOSE_ALARM_PREFIX)).map((alarm) => chrome.alarms.clear(alarm.name)));
}
...
async function rescheduleTabAlarms() {
  if (await isProtectionPaused()) { await clearAllTabAlarmState(); return; }
  const tabs = await chrome.tabs.query({});
  await Promise.all(tabs.map(async (tab) => {
    if (!Number.isInteger(tab.id)) return;
    const site = tab.url && await getSiteForTab(tab);
    if (!site) { await chrome.alarms.clear(warningAlarmName(tab.id)); await chrome.alarms.clear(closeAlarmName(tab.id)); return; }
    await scheduleTabAlarms(tab.id, await getLastActivity(tab.id), site);
  }));
}

chrome.runtime.onInstalled.addListener((details) => {
  configureIdleDetection(); rescheduleTabAlarms(); broadcastStatus();
  if (details.reason === "install") chrome.runtime.openOptionsPage();
});
chrome.runtime.onStartup.addListener(() => { configureIdleDetection(); rescheduleTabAlarms(); broadcastStatus(); });
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo) => { if (changeInfo.status === "complete") { markTabActive(tabId); broadcastStatus(); } });
chrome.tabs.onRemoved.addListener(async (tabId) => { await chrome.storage.session.remove([lastActivityKey(tabId), inheritedSiteKey(tabId)]); await chrome.alarms.clear(warningAlarmName(tabId)); await chrome.alarms.clear(closeAlarmName(tabId)); });

chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "edgeclose-activity" && Number.isInteger(sender.tab?.id)) { markTabActive(sender.tab.id); return; }
  if (message.type === "edgeclose-deadline" && Number.isInteger(sender.tab?.id)) { handleDeadlineSignal(sender.tab.id); return; }
  ...
  if (message.type === "edgeclose-resume") { resumeProtection().then(() => sendResponse({ ok: true })).catch(() => sendResponse({ ok: false })); return true; }
  if (message.type === "edgeclose-popup-state") { getPopupState().then(sendResponse).catch(() => sendResponse(null)); return true; }
  if (message.type === "edgeclose-admin-state") { getAdminState().then(sendResponse).catch(() => sendResponse(null)); return true; }
  if (message.type === "edgeclose-audit-log") { getAuditLog().then(sendResponse).catch(() => sendResponse([])); return true; }
  if (message.type === "edgeclose-audit") { appendAudit(String(message.event || "unknown").slice(0, 40), message.metadata || {}).then(() => sendResponse({ ok: true })).catch(() => sendResponse({ ok: false })); return true; }
});

chrome.storage.onChanged.addListener((changes, areaName) => {
  if ((areaName === "local" || areaName === "managed") && (changes.sites || changes.patterns || changes.timeoutSeconds || changes.warningSeconds)) {
    clearInheritedSites(); clearAllTabAlarmState(); configureIdleDetection(); rescheduleTabAlarms(); broadcastStatus();
  }
  if (areaName === "local" && changes[PAUSE_KEY]) { rescheduleTabAlarms(); broadcastStatus(); }
});

chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === PAUSE_EXPIRY_ALARM) { await resumeProtection(); return; }
  if (alarm.name.startsWith(WARNING_ALARM_PREFIX)) { await broadcastStatus(); return; }
  if (alarm.name.startsWith(CLOSE_ALARM_PREFIX)) { const tabId = Number(alarm.name.slice(CLOSE_ALARM_PREFIX.length)); if (Number.isInteger(tabId)) await closeTabIfStillInactive(tabId); return; }
  if (alarm.name === CHECK_ALARM) { await rescheduleTabAlarms(); await broadcastStatus(); }
});
